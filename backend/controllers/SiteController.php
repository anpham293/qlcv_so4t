...
class SiteController extends Controller
{
                ...
                'rules' => [
                    [
                        ...
                        ,"xuatbaocaocanhbaovungdich","xuatbaocaocanhbaovungdichtheothoigian",'download','updateseen','updatesent','notification',
                            'viewcongvan','getnhanvienphongban','getnhanvienphongbanforcheckbox','groups','updatecongviecstatus','updateduanstatus'
                            ],
                        'allow' => true,
                    ],
                    [
    ...
    public function actionQuantri()
    {
        ...
        if(isset($_GET['phongbanid'])){
            $searchModel->truongphongphutrach=$_GET['phongbanid'];
        }
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);

        $searchModel2 = new DuannSearch();
        $searchModel2->status=1;
        if(isset($_GET['phongbanid'])){
            $searchModel2->truongphongphutrach=$_GET['phongbanid'];
        }
        $dataProvider2 = $searchModel2->search(Yii::$app->request->queryParams);

        $searchModel3 = new DuannSearch();
        $searchModel3->status=2;
        if(isset($_GET['phongbanid'])){
            $searchModel3->truongphongphutrach=$_GET['phongbanid'];
        }
        $dataProvider3 = $searchModel3->search(Yii::$app->request->queryParams);

        $searchModel4 = new DuannSearch();
        $searchModel4->status=3;
        if(isset($_GET['phongbanid'])){
            $searchModel4->truongphongphutrach=$_GET['phongbanid'];
        }
        $dataProvider4 = $searchModel4->search(Yii::$app->request->queryParams);

        $searchModel5 = new DuannSearch();
        $searchModel5->status=4;
        if(isset($_GET['phongbanid'])){
            $searchModel5->truongphongphutrach=$_GET['phongbanid'];
        }
...
//            exit;
    ...
    public function actionGetnhanvienphongbanforcheckbox(){
        if(isset($_POST['id'])){
           if(isset($_POST['modelid'])){
               $return=[];
               $model = Nguoiphoihop::find()->where(['congviecid'=>$_POST['modelid']])->select("nguoiduocgan")->all();
               foreach ($_POST['id'] as $data){
                   $phongban = Phongban::findOne($data);
                   $admin = Admin::find()->where(['phongban_id'=>$data])->all();
               ...
               return $this->renderPartial('getnhanvienphongbanforcheckboxwidthmodel',['data'=>$return,'listchon'=>$model]);
           }else{
               $return=[];
               foreach ($_POST['id'] as $data){
                   $phongban = Phongban::findOne($data);
    ...
    public function actionGroups(){
        Yii::$app->response->format = Response::FORMAT_JSON;
        return [
            ["value"=>"0","text"=>"Đang thực hiện"],
            ["value"=>"1","text"=>"Đã hoàn thành"],
            ["value"=>"2","text"=>"Đang tạm dừng"],
            ["value"=>"3","text"=>"Đã hủy"],
            ["value"=>"4","text"=>"Chưa thực hiện"]
        ];
    }
    public function actionUpdatecongviecstatus(){
        $congviec = Congviec::find()->where(['id'=>$_POST['pk']])->one();
        if(!is_null($congviec)){
            $congviec->status=$_POST['value'];
            $congviec->update();
            $model=$congviec;
            Notification::push(
                Yii::$app->user->identity->id,
    ...
    public function actionUpdateduanstatus(){
        $congviec = Duan::find()->where(['id'=>$_POST['pk']])->one();
        if(!is_null($congviec)){
            $congviec->status=$_POST['value'];
            $congviec->update();
            $model=$congviec;
