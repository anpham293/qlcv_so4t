...
<tr <?php if($tiento=="")echo "style='background:#bdf0c8'"?>><td style="font-weight: bolder"><?=$tiento?><?=$index+1?></td><td>
        <strong class="<?=($value->parent==-1?"caption-subject font-red-flamingo bold":"caption-subject font-red-flamingo bold")?>"><?php
    ...
    <td><?=nl2br($value->ketquadatduoc)?></td>
    <td>
        <?php
    ...
    <td><?=($value->filedinhkem!="")?\yii\helpers\Html::a(explode("/",$value->filedinhkem)[2],'/admin/site/viewcongvan?r='.$value->filedinhkem,['data-pjax'=>0,'target'=>'_blank',"role"=>'modal-remote', "data-toggle"=>'tooltip' ]):""?></td>
    <td> <?php
        ...
        ?></td>
    <td><a class="btn btn-default" style="float: right;display: block" href="/admin/baocaocongviec/create?ids=<?=$model->id?>&parent=<?=$value->id?>" title="Tạo mới báo cáo" role="modal-remote"><i class="glyphicon glyphicon-plus"></i> <?="Thêm chi tiết"?></a>
        <?php if($value->nguoibaocao==Yii::$app->user->id || Yii::$app->user->identity->username=="Superadmin"):?><a class="btn btn-default" style="float: right;display: block" href="/admin/baocaocongviec/update?id=<?=$value->id?>" title="Tạo mới báo cáo" role="modal-remote"><i class="glyphicon glyphicon-edit"></i> Sửa báo cáo</a>
            <a class="crud-datatable-action-del btn btn-default" href="/admin/baocaocongviec/delete?id=<?=$value->id?>" title="Delete" data-pjax="false" data-pjax-container="crud-datatable-pjax" role="modal-remote" data-request-method="post" data-toggle="tooltip" data-confirm-title="Bạn có chắc không?" data-confirm-message="Bạn có chắc chắn muốn xóa mục này"><span class="glyphicon glyphicon-trash"></span></a>
        <?php endif;?>
    </td>
</tr>
