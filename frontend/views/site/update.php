...
<div class="row">
    <div class="container" id="maincontent">
        ...
        <div class="text-center">
            ĐÂY LÀ TÀI LIỆU QUAN TRỌNG, THÔNG TIN CỦA ANH/CHỊ SẼ GIÚP CƠ QUAN Y TẾ QUẢN LÝ THÔNG TIN HỒ SƠ SỨC KHỎE TOÀN
            DÂN
        </div>
        <div class="red-title text-center">
            Khuyến cáo: Khai báo thông tin sai là vi phạm pháp luật Việt Nam và có thể xử lý hình sự. Bạn đang thực hiện cập nhật với IP <?=Yii::$app->session['acceptIPAdress']?>
        </div>
            ...
            <div class="dulieuhososuckhoe-form">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 col-xs-12" style="border: 2px solid black!important; padding: 5px 15px;">
                        <?= $form->field($model, 'mahogiadinh', ['template' => '<div class="col-xs-4">{label}{error}</div><div class="col-xs-8">{input}</div>'])->numberInput(['maxlength' => true,'disabled'=>'disabled', 'placeholder' => $model->getFirstError('mahogiadinh')]) ?>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 col-xs-12">
                        <?= $form->field($model, 'socmnd', ['template' => '<div class="col-xs-4">{label}{error}</div><div class="col-xs-8">{input}</div>'])
                            ->numberInput(['maxlength' => true,'disabled'=>'disabled', 'placeholder' => $model->socmnd]) ?>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6 col-xs-12">
                        <?= $form->field($model, 'hovaten', ['template' => '<div class="col-xs-4">{label}{error}</div><div class="col-xs-8">{input}</div>'])->textInput(['disabled'=>'disabled','maxlength' => true, 'placeholder' => $model->getFirstError('hovaten')]) ?>
                    </div>
                    <div class="col-md-6 col-xs-12">
...
<script>
    $(document).ready(function () {
        ...
        $("#dulieuhososuckhoe-ngaysinh").inputmask("d/m/y", {
            autoUnmask: true,
            "placeholder": "dd/mm/yyyy",
            removeMaskOnSubmit:false
        });
        $("#dulieuhososuckhoe-ngaycap").inputmask("d/m/y", {
            autoUnmask: true,
            "placeholder": "dd/mm/yyyy",
            removeMaskOnSubmit:false
        });
        $("#dulieuhososuckhoe-thoigiantiepxuc").inputmask("d/m/y", {
            autoUnmask: true,
            "placeholder": "dd/mm/yyyy",
            removeMaskOnSubmit:false
        });
        ...
        $("#dulieuhososuckhoe-tinhthanhphohktt").on("change",function () {
            var self=$(this);
            $.ajax({
                ...
                success: function (datas) {
                    ...
                    $("#dulieuhososuckhoe-quanhuyenhktt").empty().select2({
                        data: dataDrop,
                    }).trigger('change');
        ...
        $("#dulieuhososuckhoe-quanhuyenhktt").on("change",function () {
            var self=$(this);
            $.ajax({
                ...
                success: function (datas) {
                    ...
                    $("#dulieuhososuckhoe-xaphuonghktt").empty().select2({
                        data: dataDrop,
                    }).trigger('change');
        ...
        $("#dulieuhososuckhoe-tinhthanhphonoht").on("change",function () {
            var self=$(this);
            $.ajax({
                url:"<?=Yii::$app->urlManager->createUrl(["site/getquanhuyenbytinhthanh"])?>",
                type: 'post',
                ...
                success: function (datas) {
                    ...
                    $("#dulieuhososuckhoe-quanhuyennoht").empty().select2({
                        data: dataDrop,
                    }).trigger('change');
        ...
        $("#dulieuhososuckhoe-quanhuyennoht").on("change",function () {
            var self=$(this);
            $.ajax({
                url:"<?=Yii::$app->urlManager->createUrl(["site/getphuongxabyquanhuyen"])?>",
                type: 'post',
                ...
                success: function (datas) {
                    ...
                    $("#dulieuhososuckhoe-xaphuongnoht").empty().select2({
                        data: dataDrop,
                    }).trigger('change');
